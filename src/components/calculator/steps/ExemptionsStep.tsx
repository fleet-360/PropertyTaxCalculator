...
/** Inline yes/no radio group — labels above the radios, RTL "כן" first. */
function YesNoRadio({
  value,
  onChange,
  ariaLabel,
  ...
  return (
    <RadioGroup
      ...
      value={value ?? ""}
      onChange={(_, v) => onChange(v as YesNo)}
      sx={{ gap: 3, flexShrink: 0 }}
    >
      <FormControlLabel
        value="yes"
        control={
          <Radio
            size="small"
            sx={(theme) => ({
              "&.Mui-checked": { color: theme.palette.brand.blueLight },
            })}
          />
        }
        label="כן"
...
export default function ExemptionsStep({ state, dispatch, sx }: StepProps) {
  ...
  const needsHouseholdSize = useMemo(() => {
    return rows.some((row) => {
      ...
      const section = exemptionSections.find(
        (s) => s.sectionCode === row.sectionCode,
      );
      const sub = section?.subSections?.find(
        (s) => s.code === row.subSectionCode,
      );
  ...
  const handleRemoveRow = (index: number) => {
    ...
    updateRows(
      newRows.length > 0 ? newRows : [{ sectionCode: "", subSectionCode: "" }],
    );
  };
  ...
  const setMeasurementToggle = (v: YesNo) => {
    dispatch({ type: "UPDATE_FIELD", field: "hasMeasurementError", value: v });
    if (v === "no") {
      dispatch({ type: "SET_MEASUREMENT_ERROR", payload: null });
    }
  };

  const setExemptionsToggle = (v: YesNo) => {
    dispatch({ type: "UPDATE_FIELD", field: "hasExemptions", value: v });
    if (v === "no") {
      dispatch({ type: "SET_SELECTED_EXEMPTIONS", payload: [] });
      dispatch({ type: "UPDATE_FIELD", field: "householdSize", value: 1 });
      dispatch({ type: "UPDATE_FIELD", field: "childrenCount", value: 0 });
    }
  };

  const setMultipleClassToggle = (v: YesNo) => {
    dispatch({
      type: "UPDATE_FIELD",
      field: "hasMultipleClassifications",
      value: v,
    });
  };
  ...
  const handleNext = () => {
    ...
    // Persist whatever we have so far on the lead before any branching.
    ...
    // Multi-classification properties can't be calculated automatically — send
    // the user to the contact redirect flow so a human representative can help.
    if (multipleClassToggle === "yes") {
      dispatch({
        type: "SET_CONTACT_REDIRECT",
        payload: "multiple_classifications",
      });
      return;
    }
  ...
  const conditionalSectionSx = { mt: 2 } as const;
    ...
    <Box sx={{ ...sx, justifyContent: "space-between" }}>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        >
          <Box>
            <Typography sx={wizardSectionTitleSx}>
              האם יש לנכס מספר סיווגים?
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
              לדוגמה תעשייה ומגורים, או עירוב סיווגים אחר.
            </Typography>
          </Box>
          <YesNoRadio
            value={multipleClassToggle}
            onChange={setMultipleClassToggle}
            ariaLabel="האם יש לנכס מספר סיווגים"
          />
        </Box>
      </Box>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        >
          <Typography sx={wizardSectionTitleSx}>
            האם יש לך פטור כלשהו (נכה, גמלאי, עולה, חייל, הורה יחיד, הבטחת הכנסה
            וכו&apos;) ?
          </Typography>
          <YesNoRadio
            value={exemptionsToggle}
            onChange={setExemptionsToggle}
            ariaLabel="האם יש לך פטור כלשהו"
          />
        </Box>
      </Box>

      {exemptionsToggle === "yes" && (
        <Alert severity="info" sx={{ mb: 3 }}>
          שים לב: לא ניתן לקבל כפל הנחות. תחול ההנחה הגבוהה ביותר בלבד.
        </Alert>
      )}

      {exemptionsToggle === "yes" && exemptionSections.length === 0 && (
              ...
              >
                ...
                >
                  ...
                  {exemptionSections.map((section) => (
                    <MenuItem
                      key={section.sectionLabel + section.sectionCode}
