...
const MESSAGES: Record<
  ContactRedirectStepProps["reason"],
...
export default function ContactRedirectStep({
  reason,
  dispatch,
  state,
}: ContactRedirectStepProps) {
  ...
  return (
    <Box>
      ...
      {reason === "error" && state.contactRedirectErrorMessage && (
        <Alert severity="error" sx={{ mb: 4, fontSize: "0.95rem" }}>
          {state.contactRedirectErrorMessage}
        </Alert>
      )}
